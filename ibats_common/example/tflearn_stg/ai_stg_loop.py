...
class AIStg(StgBase):
    ...
    def on_prepare_min1(self, md_df, context):
        ...
        self.load_train_test(md_df, enable_load_model=self.enable_load_model_if_exist)

    def load_train_test(self, md_df, enable_load_model, rebuild_model=False, enable_train_if_load_not_suss=True, enable_train=True, enable_test=True):
        if rebuild_model:
    ...
    def on_min1(self, md_df, context):
        ...
        if days_after_last_train >= self.retrain_period:
            logger.info('当前日期 %s 距离上一次训练 %s 已经过去 %d 天',
                        trade_date, self.trade_date_last_train, days_after_last_train)
            self.load_train_test(md_df, rebuild_model=True, enable_load_model=False, enable_train=True)
